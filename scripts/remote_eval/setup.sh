...
setup_cosmos3() {
    ...
    # The pinned Cosmos revision's lockfile was generated by an older uv and
    # current uv may consider its metadata stale. Preserve the audited lockfile
    # exactly instead of rewriting upstream source state.
    uv sync --project "$source_dir" --frozen --all-extras --group "$COSMOS_CUDA_GROUP"
    install_project_adapter "${source_dir}/.venv/bin/python"
